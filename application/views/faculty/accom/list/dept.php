...
<h3>
	...
	<div class="btn-toolbar pull-right" role="toolbar">
		<?php if ($filtered): ?>
		<div class="btn-group">
			<a class="btn btn-default" href=<?php echo url::site('faculty/accom_dept/unfilter'); ?> role="button">Remove Filter</a>
		</div>
		<?php elseif (count($ar_dept)>0) : ?>
		<div class="btn-group">
...
if (count($ar_dept)>0)
{
	...
	foreach ($ar_dept as $accom)
	{
		if (($accom->status == 'Pending') OR ($accom->status == 'Approved'))
		{
			...
			echo '<tr>';
			echo '<td><a href='.url::site('accom/view_dept/'.$accom->accom_ID).'>', $month_year, '</a></td>';
